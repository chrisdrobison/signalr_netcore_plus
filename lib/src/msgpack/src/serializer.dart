...
class Serializer {
  ...
  void encode(dynamic d) {
    ...
    if (d is ByteData) return _writeBinary(d.buffer.asUint8List(d.offsetInBytes, d.lengthInBytes));
  ...
  void _writeString(String s) {
    ...
    if (length <= 31) {
      ...
      throw FormatError('String is too long to be serialized with messagePack.');
    }
  ...
  bool _writeExt(dynamic object) {
    ...
    if (type != null) {
      ...
      if (encoded == null) throw FormatError('Unable to encode object. No Encoder specified.');
